...
class App extends Component {
  ...
  changeStateView = tabId => {
    this.setState({activeId: tabId})
  }

  thumbnailView = () => {
    const {activeId} = this.state
    const displayList = imagesList.filter(each => each.category === activeId)
    return displayList
  }
  ...
  showResponse = id => {
    ...
    if (id === image.id) {
      ...
      this.setState({image: imagesList[randomValue]})
    } else {
      clearInterval(this.timerID)
      this.setState({isGameRunning: false})
    }
  ...
  runningView = () => {
    const {image} = this.state
    const renderingList = this.thumbnailView()
    return (
      <div>
        <div>
          <img src={image.imageUrl} alt="match" />
        </div>
        <ul>
          {tabsList.map(each => (
            <li key={each.tabId}>
              <button
                type="button"
                onClick={() => this.changeStateView(each.tabId)}
              >
                {each.displayText}
              </button>
            </li>
        ...
        <ul>
          {renderingList.map(each => (
            <li key={each.id}>
              <button type="button" onClick={() => this.showResponse(each.id)}>
  ...
  changeView = () => {
    this.setState(
      {timer: 60, score: 0, isGameRunning: true},
  ...
  render() {
    const {score, timer, isGameRunning} = this.state
    const result = isGameRunning ? this.runningView() : this.resultView()
    return (
      <div>
        <ul className="bg">
          <li>
